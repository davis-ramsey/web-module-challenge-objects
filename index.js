...
const hotdog = createMenuItem('hot dog', 5, 'lunch');
const frenchFries = createMenuItem('french fries', 2, 'lunch');
...
burger.discount = function(string) {
	...
	if (string === 'public') return this.price * 0.9;
};
...
reviews.push({ name: 'Davis', rating: 5, feedback: 'Absolutely fabulous!' });
// console.log(reviews);
...
function getLastReview(reviews) {
	const { name, rating, feedback } = reviews.pop();
	return `${name} gave the restaurant a ${rating}, and their feedback was: "${feedback}"`;
}
...
function getReviewByRating(reviews, rating) {
	...
	for (let review of reviews) if (rating === Math.floor(review.rating)) filteredReviews.push(review);
...
function getLongReviews(reviews) {
	const filteredReviews = [];
	for (let review of reviews) if (review.feedback.split(' ').length > 15) filteredReviews.push(review);
	return filteredReviews;
}
